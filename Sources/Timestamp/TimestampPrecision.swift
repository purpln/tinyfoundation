...
internal extension timespec {
    ...
    init<T: BinaryFloatingPoint>(_ interval: T, precision: TimestampPrecision) {
        let seconds: time_t
        let nanoseconds: CInt
        switch precision {
        case .seconds:
            let (integer, decimal) = modf(interval)
            seconds = time_t(integer)
            nanoseconds = CInt(clamping: Int(decimal * 1e9)) % 1_000_000_000
        case .milliseconds:
            seconds = time_t(interval * 1e-3)
            nanoseconds = CInt(clamping: Int(interval * 1e6)) % 1_000_000_000
        case .microseconds:
            seconds = time_t(interval * 1e-6)
            nanoseconds = CInt(clamping: Int(interval * 1e3)) % 1_000_000_000
        case .nanoseconds:
            seconds = time_t(interval * 1e-9)
            nanoseconds = CInt(clamping: Int(interval)) % 1_000_000_000
        }
        self = timespec(tv_sec: seconds, tv_nsec: nanoseconds)
    }
    ...
    init<T: BinaryInteger>(_ interval: T, precision: TimestampPrecision) {
        ...
        case .milliseconds:
            seconds = time_t(interval) / 1_000
            nanoseconds = (CInt(clamping: interval) % 1_000) * 1_000_000
        case .microseconds:
            seconds = time_t(interval) / 1_000_000
            nanoseconds = (CInt(clamping: interval) % 1_000_000) * 1_000
        case .nanoseconds:
            seconds = time_t(interval) / 1_000_000_000
            nanoseconds = CInt(clamping: interval) % 1_000_000_000
        }
        self = timespec(tv_sec: seconds, tv_nsec: nanoseconds)
    }
#endif
