public struct Vector3<Scalar: BinaryFloatingPoint & Sendable>: Vectorable, Hashable, Sendable {
    ...
    */
    ...
    public static func * (lhs: Self, rhs: some BinaryFloatingPoint) -> Self {
        Self(lhs.x * Scalar(rhs), lhs.y * Scalar(rhs), lhs.z * Scalar(rhs))
    }

    public static func * (lhs: some BinaryFloatingPoint, rhs: Self) -> Self {
        Self(Scalar(lhs) * rhs.x, Scalar(lhs) * rhs.y, Scalar(lhs) * rhs.z)
    }
    ...
    public static func minimum(_ lhs: Self, _ rhs: Self) -> Self {
        Self(min(lhs.x, rhs.x), min(lhs.y, rhs.y), min(lhs.z, rhs.z))
    }

    public static func maximum(_ lhs: Self, _ rhs: Self) -> Self {
        Self(max(lhs.x, rhs.x), max(lhs.y, rhs.y), max(lhs.z, rhs.z))
    }
}
